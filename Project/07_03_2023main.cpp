...
int main(int argc, char** argv) {
    ...
    for (int k = 0; k < edges_in_CLT.size(); ++k) {
        // Eigen::MatrixXd Omega_k = Lambda.block(k * DOF_3, k * DOF_3, DOF_3, DOF_3).inverse();
        // Omega.block(k * DOF_3, k * DOF_3, DOF_3, DOF_3) = Omega_k;
        ...
        Eigen::MatrixXd Omega_k = J_k1.inverse().transpose() * Lambda_k1k2 * J_k2.inverse();
        cout << "Omega_k(k = " << k << "):" << endl
             << Omega_k << endl
             << endl;
        ...
        while (KLD_gradient > threshold) {
            ...
            for (int i = 0; i < edges_in_CLT.size(); ++i) {
                if (i != k) {
                    ...
                    Eigen::MatrixXd Omega_i = Omega.block(i * DOF_3, i * DOF_3, DOF_3, DOF_3);
                    cout << "Omega_i: " << endl
                         << Omega_i << endl
                         << endl;
                    Upsilon_k += J_ks[i].transpose() * Omega_i * J_ks[i];
                }
            }
            ...
            Eigen::MatrixXd Omega_k = Phi_ks[k] - (J_ks[k] * Upsilon_k.inverse() * J_ks[k].transpose()).inverse();

            Eigen::EigenSolver<Eigen::MatrixXd> eig_solver(Omega_k);
            Eigen::MatrixXd Omega_k_eig_mat = eig_solver.pseudoEigenvalueMatrix();
            // Eigen::MatrixXd Omega_k_eig_diag = Omega_k_eig_mat.diagonal().transpose();
            ...
            for (int i = 0; i < Omega_k_eig_mat.row(0).size(); ++i) {
                double eigen_value = Omega_k_eig_mat(i, i);
                if (eigen_value < epsilon) {
                    is_positive_definite = false;
                    Omega_k_eig_mat(i, i) = 0;
                }
            }
            cout << "Omega_k_eig_mat(k = " << k << "):" << endl
                 << Omega_k_eig_mat << endl
                 << endl;

            if (is_positive_definite != true) {
                Omega_k = V * Omega_k_eig_mat * V.transpose();
            }
            Omega.block(k * DOF_3, k * DOF_3, DOF_3, DOF_3) = Omega_k;
            cout << "Updated Omega: " << endl
                 << Omega << endl
                 << endl;
    ...
    Eigen::MatrixXd I_spars_subspace = J.transpose() * Omega * J;
    Eigen::MatrixXd I_spars = U * I_spars_subspace * U.transpose();
    cout << "I_spars: " << endl
         << I_spars << endl
         << endl;
    ...
    cout << "I_spars_subspace's row size: " << I_spars_subspace.row(0).size() << endl;
    cout << "I_spars_subspace's column size: " << I_spars_subspace.col(0).size() << endl;
    ...
    Eigen::Product prod = I_spars_subspace * Sigma;
    // Eigen::Product prod = I_spars * Sigma;
    double prod_det = abs(prod.determinant());
    cout << "prod.trace: " << prod.trace() << endl;
    cout << "prod.det: " << prod_det << endl;

    double KLD = 0.5 * (prod.trace() - log(prod_det) - I_spars_subspace.row(0).size());
    cout << "KLD: " << KLD << endl;
